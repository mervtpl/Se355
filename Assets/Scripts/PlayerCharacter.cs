...
public class WeaponSettings
{
    ...
    public float fireRate;              // Firing rate (in seconds)
}
...
public class PlayerCharacter : MonoBehaviour
{
    ...
    [Header("Auto Weapon System")]
    public WeaponSettings[] weapons;        // Array to add multiple weapons in the Inspector
    public float targetRange = 10f;         // Enemy detection range
    ...
    void Update()
    {
        ...
        if (!disable_controls && !is_dead)
        {
            ...
            if (controls != null)
            {
                ...
                if (controls.GetSwitchWeaponDown() && weapons != null && weapons.Length > 0)
                {
                    currentWeaponIndex++;
        ...
        if (fireTimer <= 0f && weapons != null && weapons.Length > 0 && !is_dead && !disable_controls)
        {
            Transform nearestEnemy = FindNearestEnemy();
            if (nearestEnemy != null)
            {
                Shoot(nearestEnemy);
                fireTimer = weapons[currentWeaponIndex].fireRate; // Reset the timer
            }
        }
    ...
    void Shoot(Transform target)
    {
        ...
        {
            Debug.LogError("WEAPON ERROR: Projectile Prefab is not assigned! Check the Inspector.");
            return;
        }

        if (firePoint == null)
        {
            Debug.LogError("WEAPON ERROR: Fire Point is not assigned! Projectile spawn point is unknown.");
            return;
        }

        // Instantiate the projectile
        GameObject bullet = Instantiate(weapons[currentWeaponIndex].projectilePrefab, firePoint.position, Quaternion.identity);
